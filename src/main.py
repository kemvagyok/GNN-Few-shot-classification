...
def main():
	...
	test_images = test_data.data[:100].unsqueeze(1).float() / 255.0
	test_images = test_images.to(device)
	test_targets = test_data.targets[:100]
	...
	for max_label in max_image_labeled_class:
		...
		for epoch in range(epochs_max+1):
			...
			full_graph = graph_creating(tr_te_latens_vectors, method_similar, p=p).to(device)
