...
def initialize_models(config, channel_size, num_class, device, is_ddp):
    if config.embedding == "vanilia":
        cnn = cnnModel(output_dim=64, channel_size=channel_size).to(device)
    else:
        cnn = resnetModel(output_dim=64, in_channels=channel_size, version=50).to(device)
    ...
    return cnn, gcn, opt_cnn, opt_gcn


# =========================
# Gráf létrehozása
# =========================
def build_graph_data(cnn, train_val_x, train_val_y, train_mask, config, device):
...
# Training loop
# =========================
def train_loop(cnn, gcn, data, loader, train_val_x, opt_cnn, opt_gcn, val_x, val_y, config, device, K_hop, is_ddp):
    best_acc = 0.0
    for epoch in range(config.epochs_max):
        if is_main_process() and epoch % 50 == 0:
            print(f"Epoch {epoch}/{config.epochs_max}")
        data, loader = rebuild_graph_if_needed(cnn, data, train_val_x, config, device, loader, K_hop, is_ddp)
        avg_loss = train_mini_batch(cnn, gcn, loader, train_val_x, opt_cnn, opt_gcn) if config.use_minibatch else train_full_batch(cnn, gcn, data, train_val_x, opt_cnn, opt_gcn)
        avg_loss = reduce_value(avg_loss, device)
        if is_main_process():
            print(f"Epoch {epoch}: Train Loss = {avg_loss:.4f}")
            wandb.log({"train_loss": avg_loss, "epoch": epoch})
        #Validate
        val_acc = validate(cnn, gcn, val_x, val_y, config, device)
...
def run_training(train_x, train_y, val_x, val_y, test_x, test_y, num_class, channel_size, K_hop, max_label, config, device, is_ddp=False):
    ...
    cnn, gcn, opt_cnn, opt_gcn = initialize_models(config, channel_size, num_class, device, is_ddp)
    data = build_graph_data(cnn, train_val_x, train_val_y, train_mask, config, device)
    loader = create_loader(data, train_mask, config, K_hop, is_ddp)
    best_acc = train_loop(cnn, gcn, data, loader, train_val_x, opt_cnn, opt_gcn, val_x, val_y, config, device, K_hop, is_ddp)
