...
def run_training(
    ...
    for epoch in range(config.epochs_max):
        ...
        if config.rebuild_graph_each_other:
            cnn.eval()
            latens, edge_index = build_graph(cnn, train_test_x, config, device)
            data.x = latens
...
def main():
    ...
    train_x, train_y, val_x, val_y, test_x, test_y, num_class, channel_size = loading_dataset(dataset_name, data_pth = data_path)
    ...
    if is_main_process():
        ...
        file_name = f"{results_path}/{config.CHOSE_DATASET}_run{run_id}.csv"
